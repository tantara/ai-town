// Worker-side LLM client. Reads configuration from a passed-in env object
// instead of process.env (Workers don't expose process.env).
...
export async function chatCompletion(env: LLMEnv, body: ChatRequest) {
...
export async function fetchEmbeddingBatch(env: LLMEnv, texts: string[]) {
  const config = getLLMConfig(env);
...
export async function fetchEmbedding(env: LLMEnv, text: string) {
  const { embeddings } = await fetchEmbeddingBatch(env, [text]);
  return { embedding: embeddings[0] };
}

export async function ollamaFetchEmbedding(env: LLMEnv, text: string) {
  const config = getLLMConfig(env);
